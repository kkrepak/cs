...
namespace lab1
{
		...
		static void Check(string path)
		{
			...
			Console.WriteLine("\n\nЗагальна ентропія тексту -   " + entropia);

			//кількість інформації
			double data = Data(entropia, count);
			Console.WriteLine("\n\nКількість інформації -   " + data);

			//порівняння розмірів файлу і кількості інформації
			FileInfo fi = new FileInfo(path + ".txt");
			Console.WriteLine("Кількість інформації: " + (data/1024.0) + "\nРозмір файла: " + (fi.Length) + "\nВідношення: " + (fi.Length * 1024.0 / data));
			...
			Console.WriteLine("xz - {0} ----- {1}", new FileInfo(path + ".txt.xz").Length, data);
		}
